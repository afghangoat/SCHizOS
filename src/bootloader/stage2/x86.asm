...
;int ASMCALL x86_E820GetNextBlock(E820MemoryBlock* block,uint32_t* continuation_id); //First param will be filled by BIOS
...
x86_E820GetNextBlock:
	...
	LinearToSegOffset [bp + 8], es, edi, di ;pointer to struct (es:di)
	
	LinearToSegOffset [bp + 12], ds, esi, si ;store seg (ebx), pointer to cont. id
...
.if_ok:
	;ret size
	mov eax,ecx
	mov ds:[si],ebx ;set continuation id
	jmp .end_if
